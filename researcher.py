...
class ResearcherAgent:
    ...
    async def research(self, company_url: str) -> ProspectProfile:
        """
        ...
        async with BrowserScraper() as scraper:
            ...
            pages_to_scrape = await self._plan_pages(company_url, scraper)
    ...
    async def _plan_pages(self, url: str, scraper: BrowserScraper) -> list[str]:
        """
        Scrape homepage first, then use LLM to pick the most valuable
        follow-up pages from the internal links found.

        After the LLM selects pages, a second pass scrapes listing-type pages
        (jobs, products, news) to discover deeper individual-item pages.
        """
        homepage = await scraper.scrape(url)
        pages = [url]

        if not homepage["success"]:
            logger.warning("Homepage scrape failed — will try URL-guessing fallback")
            guessed = self._guess_key_pages(url)
            return guessed[:MAX_PAGES]

        # Let LLM pick best pages from found links
        if homepage["links"]:
            links_text = "\n".join(homepage["links"][:40])
            prompt = f"""
You are a B2B sales intelligence researcher building a complete company profile.
Homepage URL: {url}

Available internal links (already ranked by relevance):
{links_text}

Your goal: select up to {MAX_PAGES - 1} URLs that together give the MOST COMPLETE
picture of this company. Cover as many distinct categories as possible:

CATEGORIES (cover every one you can find a link for):
1. Hiring / Jobs / Careers / Internships page  — reveals growth areas
2. About / Company / Mission / Story page       — company background
3. Team / Leadership / Founders page            — key people
4. Products / Features / Solutions / Platform   — what they sell
5. Pricing / Plans page                         — business model
6. Customers / Case Studies / Success Stories   — who they serve
7. News / Press / Blog / Announcements          — recent events
8. Contact / Demo / Trial page                  — sales touchpoints

RULES:
- Pick the MOST SPECIFIC URL for each category (e.g. prefer /careers over /)
- Do NOT pick two URLs from the same category
- Do NOT pick legal, cookie, or support pages
...
"""
        ...
        # Deduplicate, cap at MAX_PAGES (leave 2 slots for deep discovery)
        seen = set()
        unique = []
        for p in pages:
            if p not in seen:
        ...
        # Scrape listing pages (jobs, products, news) and pull sample
        # individual-item URLs (job descriptions, product pages, articles).
    ...
    async def _discover_deep_pages(
        ...
        """
        For listing-type pages already in the plan (jobs, products, news),
        scrape them and extract 1-2 sample individual-item URLs.

        Example:
          /careers  →  /careers/software-engineer-bangalore
          /products →  /products/enterprise-plan
          /blog     →  /blog/2024-q1-growth-update

        This is what gets us actual job descriptions and product detail text.
        """
        # Keywords that flag a page as a listing (not a leaf page)
        LISTING_KEYWORDS = [
        ...
        for page_url in planned_urls:
            ...
            is_listing = any(kw in path for kw in LISTING_KEYWORDS)
            if not is_listing:
                continue
            ...
            try:
                ...
                # Find child URLs (same domain, longer/deeper path)
                ...
                # Take up to 2 sample child pages
                for child in children[:2]:
                    deep_urls.append(child)
                    already_planned.add(child)
                    logger.info(f"   📄 Deep page discovered: {child[:70]}")
    ...
    def _guess_key_pages(self, base_url: str) -> list[str]:
        """
        Fallback: guess common page paths when LLM link planning fails.
        Covers jobs, team, product, pricing, news — all the key categories.
        """
        paths = [
            # Hiring
  ...
  "open_job_roles": [
    {
      ...
      "skills": ["string (required skills or tech mentioned)"],
      "description_snippet": "string or null (1-2 sentence summary of the role)"
    }
  ],
  "hiring_velocity": "string or null (summary e.g. '15 open roles, heavy engineering hiring')",
  "hiring_locations": ["string (unique locations where they are hiring)"],
...
"""
    ...
    async def _detect_buying_signals(
        ...
        prompt = f"""
You are a B2B sales signal analyst. Analyze the company information below and identify
...
Identify up to 6 buying signals. For each signal, be VERY SPECIFIC — reference actual
facts from the text (e.g. "Hired 3 ML engineers in Q1 2024" not just "hiring").

Valid signal types:
- hiring_surge: Rapid hiring in a specific function (name the function and count)
- skill_demand: Specific tech/skills listed repeatedly across job postings
- recent_funding: Fresh capital announced (name the round and amount)
...
- geographic_expansion: Opening new offices or expanding to new markets
...
[
  {
    ...
    "description": "specific factual detail — name names, cite numbers",
